...
class AliyunASRExtension(Extension):
    ...
    def on_start(self, ten_env: TenEnv) -> None:
        ...
        self.config = AliyunASRConfig.create(ten_env=ten_env)
        ten_env.log_info(
    ...
    def _send_text(self, text: str, is_final: bool, stream_id: int) -> None:
    ...
    def start_recognition(self):
        try:
            ...
            #     self.extension.loop,
            # )
            # thread = threading.Thread(target=self.recognition.start)
            # thread.start()
            self.recognition.start()
...
class Callback(RecognitionCallback):
    def __init__(self, ten_env: TenEnv, extension: AliyunASRExtension):
        self.ten_env = ten_env
