...
class OpenAIChatGPT:
    ...
    async def get_chat_completions_stream(self, ten_env:AsyncTenEnv, messages, tools = None, listener = None):
        req = {
        ...
        async for chat_completion in response:
            ten_env.log_info(f"received chat completion")
            choice = chat_completion.choices[0]
            delta = choice.delta
            ten_env.log_info(f"delta {delta}")
            content = delta.content if delta and delta.content else ""
            ten_env.log_info(f"received content {content}")
            # Emit content update event (fire-and-forget)
            if listener and content:
                ten_env.log_info(f"will emit content update event")
                listener.emit('content_update', content)

            full_content += content
            ten_env.log_info(f"full_content {full_content}")
